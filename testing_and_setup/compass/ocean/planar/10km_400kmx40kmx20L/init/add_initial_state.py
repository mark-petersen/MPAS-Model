...
def SpecifyInitialConditions():
    ...
    refBottomDepthArray = np.zeros(nVertLevels)
    refZMidArray = np.zeros(nVertLevels)

    for iVertLevel in range(0,nVertLevels):
        refBottomDepthArray[iVertLevel] = SGWBottomDepthParam*interfaceLocations[iVertLevel+1]
        refZMidArray[iVertLevel] = -0.5*(interfaceLocations[iVertLevel+1] + interfaceLocations[iVertLevel]) \
                                       *SGWBottomDepthParam          
    
    refBottomDepth[:] = refBottomDepthArray[:]
    refZMid[:] = refZMidArray[:]
    ...
    yMin = 10.0**10.0
    yMax = -10.0**10.0
    yMin = min(yMin,min(yCell))
    yMax = max(yMax,max(yCell))
    ...
    for iCell in range(0,nCells):
        for iVertLevel in range(0,nVertLevels):
            restingThicknessArray[iCell,iVertLevel] \
            = SGWBottomDepthParam*(interfaceLocations[iVertLevel+1] - interfaceLocations[iVertLevel])
            layerThicknessArray[iCell,iVertLevel] \
            = restingThicknessArray[iCell,iVertLevel] \
    ...
    temperatureArray = np.zeros((nCells,nVertLevels))
    
    for iCell in range(0,nCells):    
        for iVertLevel in range(0,nVertLevels):   
            iVertLevelReversed = nVertLevels - iVertLevel - 1
            temperatureArray[iCell,iVertLevelReversed] \
            = SGWBottomTemperatureParam \
              + (SGWSurfaceTemperatureParam - SGWBottomTemperatureParam) \
                *((refZMidArray[iVertLevelReversed] + refBottomDepthArray[nVertLevels-1]) \
                  /refBottomDepthArray[nVertLevels-1])
    
    temperature[0,:,:] = temperatureArray[:,:]
    salinity[:] = SGWSalinityParam
